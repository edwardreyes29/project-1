    ...
    function usMap() {
        ...
        $.ajax(settings).done(function (response) {
            ...
            async function asyncCall2() {
                for (var i = 0; i < response.locations.length; i++) {
                    ...
                    $("#total-cases-country").append("<span class='total-number-state'>" + response.locations[i].state + "</span> " + formatNumber(response.locations[i].latest.confirmed) + "</br>")
                }
            }
        ...
        $.ajax(settings).done(function (response) {
            ...
            async function asyncCall3() {
                for (var i = 0; i < response.locations.length; i++) {
                    $("#total-deaths").append(`<div class="input-search" data-toggle="modal" data-target="#exampleModal" data-state=${response.locations[i].state}><span class="total-number-state" >${response.locations[i].state}</span> ${formatNumber(response.locations[i].latest.deaths)}</div>`)
